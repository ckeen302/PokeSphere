...
const FunFact = ({
  pokemon,
  generateFunFact,
}: { pokemon: Pokemon; generateFunFact: (pokemon: Pokemon) => Promise<string> }) => {
  ...
  return <p className="text-sm italic text-[#00FFFF]/80">{isLoading ? "Loading fun fact..." : fact}</p>
}
...
export default function PokédexMode({ pokemonList }: PokédexModeProps) {
  ...
  const filteredPokemon = pokemonList
    ...
    .filter(
      (pokemon) =>
        ...
        (pokemon.name.toLowerCase().includes(filter.toLowerCase()) || pokemon.id.toString().includes(filter)),
    )
  ...
  const SelectedPokemonView = ({ pokemon, onBack }: { pokemon: Pokemon; onBack: () => void }) => (
    <div className="h-[calc(100vh-220px)]">
      <div className="flex justify-between items-center mb-4">
        ...
        />
        <Button onClick={onBack} className="bg-[rgba(24,191,191,0.2)] hover:bg-[rgba(24,191,191,0.3)]">
          Back to List
        </Button>
      </div>
                ...
                return (
                  <div key={stat.name} className="mb-2">
                    ...
                    <div className="h-2 bg-[rgba(24,191,191,0.1)] rounded-full overflow-hidden">
                      <div className="h-full bg-[#18BFBF]" style={{ width: `${(displayValue / 255) * 100}%` }} />
                    </div>
                  </div>
                )
              })}
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    const touch = e.touches[0]
    if (touch && scrollContainerRef.current) {
      const startX = Number.parseInt(scrollContainerRef.current.dataset.startX || "0")
      const scrollLeft = Number.parseInt(scrollContainerRef.current.dataset.scrollLeft || "0")

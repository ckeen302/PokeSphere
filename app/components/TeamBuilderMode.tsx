...
export default function TeamBuilderMode({ pokemonList }: TeamBuilderModeProps) {
  ...
  const PokemonItem = ({ index, style }: { index: number; style: React.CSSProperties }) => {
    ...
    const displayedPokemon =
      showMegaDynamax && (pokemon.megaForm || pokemon.dynamaxForm)
    ...
    return (
      <div style={style}>
        ...
        >
          ...
          <div className="mt-2">
            <h4 className="text-sm font-semibold mb-1 text-white">Stats:</h4>
    ...
    <div className="gameboy-content-inner">
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-col">
                <span className="font-medium text-white">{displayedPokemon?.name || member.pokemon.name}</span>
                {member.form !== "regular" && <span className="text-xs opacity-75 text-white">({member.form})</span>}
                <div className="text-xs text-white">
                  {member.pokemon.stats.map((stat) => (
                    <span key={stat.name} className="mr-2">
      ...
      <div className="filter-container">
        <div className="flex gap-4 mb-2">
          <Select value={selectedRegion} onValueChange={(value: PokedexRegion) => setSelectedRegion(value)}>
          ...
          <Select value={selectedType} onValueChange={(value: PokemonType) => setSelectedType(value)}>
            <SelectTrigger className="w-[180px] bg-[rgba(24,191,191,0.1)] border-[#18BFBF] text-white">
        ...
        <div className="mb-2">
          <MegaDynamaxToggle isEnabled={showMegaDynamax} onToggle={() => setShowMegaDynamax(!showMegaDynamax)} />
        </div>
        <Input
          type="text"
      ...
      <div className="pokemon-list" style={{ height: "calc(100vh - 250px)" }}>
        <AutoSizer>
          {({ height, width }) => (
            <List height={height} itemCount={filteredPokemon.length} itemSize={40} width={width}>
              {({ index, style }) => (
